...
def make_quiver2_plot(dipole_forces, z_plane_i, dipole_size, cube_side_length):
    ...
    _, ax = plt.subplots()
    padding = 2
    ax.set(xlabel="x", ylabel="y", xlim=(xs[0]-padding,xs[-1]+padding), ylim=(ys[0]-padding,ys[-1]+padding), title="Quiver plot")
...
def make_quiver3_plot(grid, should_normalise_arrows=True):
    ...
    ax.set(xlabel="x /μm", ylabel="y /μm", zlabel="z /μm", xlim=(x[0,0,0],x[-1,1,0]), ylim=(y[0,0,0],y[0,-1,0]), zlim=(z[0,0,0],z[0,0,-1]), title="Force on each dipole")
