...
def force_with_sep_main(cube_side_lengths, sep_min, sep_max, Lambda, dipole_size, force_per_dipole=False):
    ...
    for cube_side_length in cube_side_lengths:
        ...
        ax.plot(plot_seps, plot_Fxs, label=f"${cube_side_length}^3$ dipoles")

    
    if force_per_dipole:
        ax.set(xlabel="Cube centre separation /lattice spacings", ylabel="$F_x$ /$10^{-13}$N")
        print("F_x per dipole against separation")
    else:
        ax.set(xlabel="Cube centre separation /lattice spacings", ylabel="$F_x$ /$10^{-13}$N")
        print("F_x against separation")
    plt.legend()
    plt.savefig('force_with_sep.eps', format='eps')
    plt.show()
...
def force_with_sep_const_cube_size(cube_side_lengths, sep_min, sep_max, Lambda, cube_size, force_per_dipole=False):
    ...
    if force_per_dipole:
        ...
        print("F_x per dipole against cube separation")
    else:
        ax.set(xlabel="Separation /$10^{-6}$m", ylabel="$F_x$ /$10^{-13}$N")
        print("F_x against cube separation")

    plt.legend()
    plt.savefig('force_with_sep_const_cube_size.eps', format='eps')
    plt.show()
...
def force_with_wavelength(cube_side_length, sep_min, sep_max, Lambdas, dipole_size, force_per_dipole=False):
    ...
    if force_per_dipole:
        ax.set(xlabel="Separation in lattice spacings", ylabel="$F_x$ /$10^{-13}$N")
        print("F_x per dipole against cube separation")
    else:
        ax.set(xlabel="Separation in lattice spacings", ylabel="$F_x$ /$10^{-13}$N")
        print("F_x against cube separation")
...
def main():
    ...
    Lambda = 6.28 # e-6 m
    dipole_size = 0.418
    cube_side_lengths = [1,2,3,4,5,6] # in lattice spacings
    force_with_sep_main(cube_side_lengths, 0, 100, Lambda, dipole_size, force_per_dipole=False)
