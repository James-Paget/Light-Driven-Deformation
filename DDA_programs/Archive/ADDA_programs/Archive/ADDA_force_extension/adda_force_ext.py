...
def Generate_Geom(geom_type, geom_args):
    """
    . Creates a .geom file for the parameters given

    . Cube; [width]
        width = integer width of cube, in dipoles
    . Rectangle; [width, height]
        width = integer width of rectangle, in dipoles, for its larger surface in the XY plane
        height = integer number of dipoles in the Z axis, eg. the thin surface
    . Sphere;
        diameter = bounding diameter of sphere, binned into integer sections
        flatness = float factor that extends the radius considered, allows spheres to have flatter axial surfaces
    """
    geom_file_body = ""
    match geom_type:
        case "cube":
            width = geom_args[0]
            print("Generating cube; "+str(width)+"x"+str(width)+"x"+str(width))
            geom_file = open("cube"+str(width)+"x"+str(width)+"x"+str(width)+".geom", "w")
            for k in range(width):
                for j in range(width):
                    for i in range(width):
                        geom_file_body += str(i)+" "+str(j)+" "+str(k)+"\n"
            geom_file.write(geom_file_body)
            geom_file.close()
        case "rectangle":
            width = geom_args[0]
            height = geom_args[1]
            print("Generating rectangle; "+str(width)+"x"+str(width)+"x"+str(height))
            geom_file = open("rectangle"+str(width)+"x"+str(width)+"x"+str(height)+".geom", "w")
            ...
            geom_file.close()
        case "sphere":
            diameter = geom_args[0]
            flatness = geom_args[1]
            radius = flatness*np.ceil(diameter/2.0)
            print("Generating sphere; "+str(diameter)+"x"+str(diameter)+"x"+str(diameter))
            geom_file = open("sphere"+str(diameter)+"x"+str(diameter)+"x"+str(diameter)+".geom", "w")
            for k in range(diameter):
                for j in range(diameter):
                    for i in range(diameter):
                        if( pow(i-diameter/2.0, 2)+pow(j-diameter/2.0, 2)+pow(k-diameter/2.0, 2) -pow(radius, 2) <= sys.float_info.epsilon ):
                            geom_file_body += str(i)+" "+str(j)+" "+str(k)+"\n"
            geom_file.write(geom_file_body)
            geom_file.close()    
        ...
        for char_index in range(0, len(line)):
            ...
            match word:
                ...
                case "CoupleConstant: ":
                    ...
                    try:
                        ##
                        ## NOTE; Currently breaks if polarisability is large enough to have no 'e' term in form due to this approach --> fix this if polarisability wants to be used
                        ##
                        ...
                        print("Could NOT cast parameter to complex: polarisability");
...
def RunAdda(input_shape, input_beam, input_beam_args, dpl, n_real, n_imag, wavelength, output_folder, storeForce=False):
    beam_args = ""
    for arg in input_beam_args:
        beam_args += " "+str(arg)
    refractive_indices = str(n_real)+" "+str(n_imag)+" "+str(n_real)+" "+str(n_imag);
    adda_command = "./adda -store_force -iter csym -dpl " +str(dpl)+ " -lambda " + str(wavelength)+ " -shape read " + input_shape + " -m "+ refractive_indices + " -store_beam -store_int_field -store_dip_pol -beam "+input_beam+beam_args+" -dir " + str(output_folder);
    if(storeForce):
        adda_command += " -store_force"
...
def factorial(value):
	...
	"""
	break_condition = 50;	# Break after 50 loops to prevent inf. loop, factorials deal with here never more than ~10
	factorial_value = 1;
	if(value > 0):
...
def integrate_simpsons_rule(limit_lower, limit_upper, samples, cFunction, component, rho, phi, z, k, l, p, z_R, w0, alpha, beta):
	"""
	. Evaluates an integral using simpson's rule
	"""
	integral = 0.0 +0.0j;
	sample_step = (limit_upper-limit_lower)/(samples);

	# Goes to 0 at end points (problems if included)
	# Integral += cFunction( (limit_lower), component, rho, phi, z, k, l, p, z_R, w0, alpha, beta);
	for i_ind in range(1,samples):
...
def laguerre_getParam_w(zeta, w0):
	"""
	. Get the 'w' term used in laguerre-gaussian beam
	"""
	return w0*cmath.sqrt(1.0+(zeta*zeta));

def laguerre_getParam_Llp(value, l, p):
    L = 0.0 +0.0j;	# Generalised implementation ### UNFINISHED PLACEHOLDER ###
    L_0_8 = ( (6435.0)*pow(value,8) -(12012.0)*pow(value,6) +(6930.0)*pow(value,4) -(1260.0)*pow(value,2) +(35.0) )/128.0;
    return L_0_8;

def laguerre_getParam_Bessel(value, l):
    terms = 6;	# of terms to take in Taylor series
    bessel_value = 0.0 +0.0j;
    for i_ind in range(terms):
        set_1 = (pow(-1,i_ind))/(factorial(i_ind)*factorial(i_ind+l));
        set_2 = pow( (value/2.0), (2*i_ind +l));
        bessel_value += set_1*set_2;
    return bessel_value;

def laguerre_getParam_C(l, p):
	# (factorial(p))*sqrt( (2)/(PI*factorial(p)*( factorial( abs(l) + p ) )) );
	C_formualtion = (factorial(p))*math.sqrt( (2)/(np.pi*factorial(p)*( factorial( abs(l) + p ) )) );
	return C_formualtion;

def laguerre_getParam_Ulp(C, l, p, rho, phi, k, z, z_R, w0):
	set_1 = (C)/cmath.sqrt(1 +((z*z)/(z_R*z_R)));
	set_2 = pow( (rho*cmath.sqrt(2))/(laguerre_getParam_w(z, w0)), l);
	set_3 = laguerre_getParam_Llp((2.0*(rho*rho))/(pow(laguerre_getParam_w(z, w0),2)), l, p);
	set_4 = cmath.exp( (-(rho*rho))/(pow(laguerre_getParam_w(z, w0),2)) );
	set_5 = cmath.exp( (-1.0j*k*(rho*rho)*z)/(2.0*((z*z)+(z_R*z_R))) );
	set_6 = cmath.exp(1.0j*l*phi);
	set_7 = cmath.exp( (1.0j*(2.0*p +l+1))*(cmath.atan( (z)/(z_R) )) );
	return set_1*set_2*set_3*set_4*set_5*set_6*set_7;

def laguerre_getParam_E(kappa, rho, phi, z, k, l, p, z_R, w0):
	C = laguerre_getParam_C(l, p);  # 1.0 +0.0*I;
	set_1 = laguerre_getParam_Ulp(C, l, p, rho, phi, k, z, z_R, w0);
	set_2 = cmath.exp( (-k*(kappa*kappa)*z_R)/(2.0*((k*k) - (kappa*kappa))) );
	set_3 = pow( ((kappa*kappa))/((k*k) - (kappa*kappa)), (2.0*p+l+1.0)/(2.0) );
	set_4 = cmath.sqrt( ((k*k))/((k*k) - (kappa*kappa)) );
	set_5 = set_1*set_2*set_3*set_4;
	return set_5;

def laguerre_getParam_E_component(kappa, component, rho, phi, z, k, l, p, z_R, w0, alpha, beta):
	E = laguerre_getParam_E(kappa, rho, phi, z, k, l, p, z_R, w0);
	if(component == 0):
		set_1 = cmath.exp(1.0j*l*phi);
		set_2 = cmath.exp(1.0j*z*cmath.sqrt((k*k)-(kappa*kappa)));
		set_3 = ( alpha*laguerre_getParam_Bessel(kappa*rho, l) );
		...
		set_1 = cmath.exp(1.0j*l*phi);
		set_2 = cmath.exp(1.0j*z*cmath.sqrt((k*k)-(kappa*kappa)));
		set_3 = (kappa)/(2.0*cmath.sqrt((k*k) - (kappa*kappa)));
		set_4 = (1.0j*alpha - beta)*(cmath.exp(-1.0j*phi))*laguerre_getParam_Bessel(kappa*rho, l-1) - (1.0j*alpha + beta)*(cmath.exp(1.0j*phi))*laguerre_getParam_Bessel(kappa*rho, l+1);
...
def Calculate_IncidentField(beam_spec, pos_set):
    ...
    def Calculate_IncidentField_LaguerreGaussian_Point(pos, wavelength, radial, azimuthal, alpha, beta, w_0, showVariables=False):
        """
        ...
        def E_vector_component(component, kappa):
            ...
            if(component == 0):     #X comp
                set_1 = cmath.exp(1j*l*phi)
                set_2 = cmath.exp(1j*z*cmath.sqrt(pow(k,2)-pow(kappa,2)))
                set_3 = ( alpha*sp.jv(l, kappa*rho) )
                return E(kappa)*set_1*set_2*set_3
    ...
    for pos in pos_set:
        ...
        match beam_spec[0]:
            ...
            case "laguerre":
                # Using direct ADDA approach
                k = (2.0*np.pi) / beam_spec[1]
                z=pos[2]
                phi=math.atan2(pos[1],pos[0]);
                rho = np.sqrt( (pos[0]*pos[0]) +(pos[1]*pos[1]) );	
                ...
                E_x = integrate_simpsons_rule(0.0, k, 31, laguerre_getParam_E_component, 0, rho, phi, z, k, l, p, z_R, w0, t1, t2);
                E_y = integrate_simpsons_rule(0.0, k, 31, laguerre_getParam_E_component, 1, rho, phi, z, k, l, p, z_R, w0, t1, t2);
                E_z = integrate_simpsons_rule(0.0, k, 31, laguerre_getParam_E_component, 2, rho, phi, z, k, l, p, z_R, w0, t1, t2);
                E_field = [E_x, E_y, E_z]

                #beam_spec = [beam_type, wavelength, azi, radial, alpha, beta, w_0]
                E_field = Calculate_IncidentField_LaguerreGaussian_Point(pos, beam_spec[1], beam_spec[2], beam_spec[3], beam_spec[4], beam_spec[5], beam_spec[6], showVariables=False)
            case "plane":
                #beam_spec = [beam_type, wavelength, azi, radial, alpha, beta, w_0]
...
def Sweep_Beam_Generation(space_data, wavelength):
    ...
    for beta_sweep in range(0, N_iter):
        ...
        for alpha_sweep in range(0, N_iter):
            ...
            field_data = Calculate_IncidentField(["laguerre",wavelength,0,8, alpha,beta, w_0], space_data);
            Plot_2D_Array(
                "Z",
                "norm", 
...
def Calculate_InternalField(dipole_set, request_indices, polarisability, beam_spec, mode=0):
    """
    . Calculates the scattered internal field at the requested points from a full set of dipoles given
    ...
    # Calculate this scattered near-field at each requested point
    E_interactions = np.zeros((len(request_indices),7), dtype=complex)
    for j in range(len(request_indices)):
        r_ind = request_indices[j]
        if(j % 50 == 0):
            print("     dipole "+str(j)+"/"+str(len(request_indices)))
    ...
    if(mode == 1):
        E_interactions[j, 3:] = E_interactions[j, 3:].real    # Take real parts of all components for plots if wanted
        E_interactions = np.array(E_interactions, dtype=float)
    return E_interactions   # [x,y,z,|E|^2,E_x, E_y, E_z], Complex values
...
def Calculate_InternalField_Point(dip_polarisation, polarisability):
    """
    . Internal field for a given dip polarisation
    """
    E_interaction = dip_polarisation/polarisability
    return E_interaction   # [E_x, E_y, E_z], Complex values
...
def Calculate_TotalField_NearField_Point(pos, dip_positions, dip_polarisations, beam_spec, mode=0):
    ...
    return E_total      #[x,y,z,|E|^2,E_x,E_y,E_z]
...
def main_plot_adda_E_incident(input_shape_name, input_beam_name, input_beam_args, output_folder, wavelength, dpl, material_refractive_index):
    ...
    # Run ADDA simulation
    RunAdda(input_shape_name, input_beam_name, input_beam_args, dpl, material_refractive_index.real, material_refractive_index.imag, wavelength, output_folder, storeForce=False);
...
def main_plot_adda_E_internal(input_shape_name, input_beam_name, input_beam_args, output_folder, wavelength, dpl, material_refractive_index):
    ...
    posEField_Python_Xpol = Calculate_InternalField(posPolarisation_ADDA_Xpol, index_set, polarisability, plane_beam_spec, mode=1)
...
#laguerre_beam_spec  = ["laguerre",wavelength,0,8, 0.4*pow(10,-8)*(1.0+1j), 0.4*pow(10,-8)*(1.0-1j), 0.6*wavelength];
...
Generate_Geom("rectangle", [40,2])
#Generate_Geom("sphere", [32,1.1])
...
match(sys.argv[1]):
    ...
    case 'plot_adda_force':
        main_plot_adda_E_force("sphere16x16x16.geom", "plane", "", "output_data", 1.064, 15, 1.5+0j);
    case _:
