...
// initialize beam; produce description string
{
	...
	vorticity = 0;
	printf("LOG PRINT; ---- RUN BEAM INIT ---- \n");
	switch (beamtype) {
		case B_PLANE:
			printf("LOG PRINT; ---- RUN PLANE BEAM INIT ---- \n");
			if (IFROOT) beam_descr="plane wave";
			printf("LOG PRINT; ---- Before surf---- \n");
			if (surface) {
				printf("LOG PRINT; ---- 	INSIDE surf---- \n");
		...
		case B_DIPOLE:
			...
			return;
		case B_LAGUERRE:
			printf("LOG PRINT; Laguerre Init-> Start of case \n");
			l=beam_pars[0];
			p=beam_pars[1];
			//####################################
			//## POSSIBLY CHANGE VORTICITY HERE ##
			//####################################
			printf("LOG PRINT; Laguerre Init-> Mid of case \n");
			//if (IFROOT) {
			//	tmp_str="Laguerre-Gaussian formulation";
			//	break;
			//}
			printf("LOG PRINT; Laguerre Init-> End of case \n");
			return;
		case B_LMINUS:
		case B_DAVIS3:
		case B_BARTON5:
		...
		case B_BES_TEL:
		case B_BES_TML:
			printf("LOG PRINT; ---- Inside Bessel Beam ---- \n");
...
double complex integrate_simpsons_rule(double complex limit_lower, double complex limit_upper, int samples, double complex (*cFunction)(double complex, int, double, double, double, double, int, int, double, double, double complex, double complex), int component, double rho, double phi, double z, double WaveNum, int l, int p, double z_R, double w0, double complex alpha, double complex beta) {
	/*
	...
	*/
	...
	double complex sample_step = (limit_upper-limit_lower)/(samples);
	printf("InnerLog; (2)");

	integral += cFunction( (limit_lower), component, rho, phi, z, WaveNum, l, p, z_R, w0, alpha, beta);
	for(int i=1; i<samples; i++) {
		printf("InnerLog; (3)");
		//Loop and evaluate at the same time
		if(i%2==0) {
			//2* for evens
			integral += 2.0*cFunction( (limit_lower +i*sample_step), component, rho, phi, z, WaveNum, l, p, z_R, w0, alpha, beta);
		} else {
			//4* for odds
			integral += 4.0*cFunction( (limit_lower +i*sample_step), component, rho, phi, z, WaveNum, l, p, z_R, w0, alpha, beta);
		}
	}
	printf("InnerLog; (4)");
	integral += cFunction( (limit_upper), component, rho, phi, z, WaveNum, l, p, z_R, w0, alpha, beta);
	return integral*(sample_step/3.0);
}
double complex laguerre_getParam_w(double zeta, double w0) {
	/*
	. Get the 'w' term used in laguerre-gaussian beam
	*/
	return w0*sqrt(1.0+pow(zeta,2));
}
double complex laguerre_getParam_Llp(double calue, int l, int p) {
	/*
	. Get associated legendre polynomial of given order
	*/
	return 1.0 +0.0*I;	//#### IMPLEMENT THIS ####
}
double complex laguerre_getParam_C(int l, int p) {
	/*
	. Get normalisation constant C, used in u_l_p function
	. From “Gaussian Beams In Optics of Course” paper
	*/
	return (factorial(p))*sqrt( (2)/(PI*factorial(p)*( factorial( abs(l) + p ) )) );
}
double complex laguerre_getParam_Ulp(double C, int l, int p, double rho, double phi, double WaveNum, double z, double z_R, double w0) {
	/*
	. Get u_l_p, which describes behaviour of laguerre-gaussian beam
	*/
	return ( (C)/sqrt(1 +(pow(z,2)/pow(z_R,2))) )*pow( (rho*sqrt(2))/(laguerre_getParam_w(z, w0)), l)*(laguerre_getParam_Llp((2.0*pow(rho,2))/(pow(laguerre_getParam_w(z, w0),2)), l, p))*(cexp( (-pow(rho,2))/(pow(laguerre_getParam_w(z, w0),2)) ))*(cexp( (-I*WaveNum*pow(rho,2)*z)/(2.0*(pow(z,2)+pow(z_R,2))) ))*(cexp(I*l*phi))*(cexp( (I*(2.0*p +l+1))*(atan( (z)/(z_R) )) ));
}
double complex laguerre_getParam_E(double kappa, double rho, double phi, double z, double WaveNum, int l, int p, double z_R, double w0) {
	/*
	. Get E magnitude term for laguerre-gaussian beam
	*/
	printf("	Inside _E func  (a)");
	//double C = laguerre_getParam_C(l, p);
	printf("	Inside _E func  (b)");
	return 0.0;//laguerre_getParam_Ulp(C, l, p, rho, phi, WaveNum, z, z_R, w0)*cexp( (-WaveNum*pow(kappa,2)*z_R)/(2.0*(pow(WaveNum,2) - pow(kappa,2))) )*pow( (pow(kappa,2))/(pow(WaveNum,2) - pow(kappa,2)), (2.0*p+l+1.0)/(2) )*sqrt( (pow(WaveNum,2))/(pow(WaveNum,2) - pow(kappa,2)) );
}
double complex laguerre_getParam_E_component(double kappa, int component, double rho, double phi, double z, double WaveNum, int l, int p, double z_R, double w0, double complex alpha, double complex beta) {
	/*
	. Returns magnitude of the ith component of E (x,y or z)
	*/
	printf("Inside E Comp \n");
	if(component == 0) {
		double temp_v0 = kappa*rho;
		...
		printf("temp_v2= %f \n", temp_v2);
		printf("temp_v3= %f \n", temp_v3);
		printf("MIDDLE of 0 E \n");
		//void bjndd_(const int *n,const double *x,double *bj,double *dj,double *fj);
		bjndd_(&l, &temp_v0, &temp_v1, &temp_v2, &temp_v3 );		//Bessel function, only care about t2, not t3 or 4	//##### CAREFUL OF TYPES FOR THIS RETURN #####
		printf("AFTER BESSEL 0 E \n");
		double complex E_x = laguerre_getParam_E(kappa, rho, phi, z, WaveNum, l, p, z_R, w0);
		printf("AFTER AFTER BESSEL --> X2");
		return E_x*cexp(I*l*phi)*cexp(I*z*sqrt(pow(WaveNum,2)-pow(kappa,2)))*( alpha*(temp_v1) );
	} else if(component == 1) {
		return 1.0;
	} else if(component == 2) {
		return 2.0;
...
int factorial(int value) {
	...
	*/
	...
	int factorial_value = value;
	while(factorial_value > 0) {
		value = value-1;
...
// generates incident beam at every dipole
{
	...
	double rho;		//phi already given below
	doublecomplex t1,t2,t3,t4,t5,t6,t7,t8,ctemp;	//Temporary double complex
	double z_R;		//Laguerre temp.
	...
	switch (beamtype) {
		case B_PLANE: // plane is separate to be fast (for non-surface)
			if (surface) {
				...
				printf("LOG PRINT; ---- Gen beam; non-surf---- \n");
				j=3*i;
			...
			printf("LOG PRINT; ---- Gen beam; return---- \n");
			return;
		case B_DIPOLE: {
...
//				tmp=cDotProd_Im(v2,v1);
		...
		case B_LAGUERRE:
			...
			cvMultScal_RVec()    => Multiplies a !doublecomplex SCALAR! by a !doublecomplex VECTOR!, and stores the result in a !doublecomplex VECTOR!
			...
			*/
			...
			for (i=0;i<local_nvoid_Ndip;i++) { // standard (non-surface) plane wave
				...
				#######
				=====================================================================================================
				====================== NEED TO WRITE ASSOCIATED LEGENDRE POLNOMIAL FORMULATION ======================
				=====================================================================================================
				//######
				//###### LOTS OF VALUES ARE 0 --> THIS IS THE MAIN PROBLEM
				//######		--> NEED TO PULL FROM LAMBDA IN params.c
				##
				##	-----> This could be a print problem --> wrong %.. format gives 0s
				##		   printing ints I just made seem fine, but from variables parsed in functions I can get problem
				##
				//######
				...
				########
				#### stack overflow error --> split into smaller values
				####		Deep chains are not good --> try split up a bit more
				####	The error can also come from wrong data types being used / cast to, causing undefined errors
				########
				*/
				printf("LAGUERRE LOG; =======> \n");
				printf("LAGUERRE LOG; =======> %f \n",r1);
				printf("LOG PRINT; ---- Gen beam; LAGUERRE Dipole---- \n");
				printf("LAGUERRE LOG; i= %i \n",i);
				printf("LAGUERRE LOG; DipoleCoord= %f \n",DipoleCoord);
				printf("LAGUERRE LOG; beam_center= %f \n",beam_center);
				printf("LAGUERRE LOG; r1= %f \n",r1);
				
				vSubtr(DipoleCoord+j,beam_center,r1);	//<--- Give it the X coord index, it will pull out the +1 & +2 coords in this operation, find the diff with the beam_center vector, and store in r1, a double[3]

				printf("LAGUERRE LOG; r1= %f \n",r1);

				double wavelength = (2.0*PI)*(WaveNum);
				double w0 = 0.6*wavelength;					//<--- Was defined elsewhere for other beams (as a param, maybe worth doing same here)
				
				printf("LAGUERRE LOG; WaveNum= %f \n",WaveNum);
				printf("LAGUERRE LOG; wavelength= %f \n",wavelength);
				printf("LAGUERRE LOG;         w0= %f \n",w0);
				
				rho = sqrt( pow(x,2) + pow(y,2) + pow(z,2) );		//Looks like vNorm() will do this but I would have to put all elements into a vector
				phi = atan2(y,x);		//
				z   = z;				//
				t1  = 1.0+1.0*I; 		//alpha
				t2  = 1.0-1.0*I; 		//beta
				z_R = WaveNum*pow(w0, 2)/2.0;	//z_R
				printf("LAGUERRE LOG; rho= %f \n",rho);
				printf("LAGUERRE LOG; phi= %f \n",phi);
				printf("LAGUERRE LOG;   z= %f \n",z);
				printf("NewLog; t1= %f, %f \n",creal(t1), cimag(t1));
				printf("NewLog; t2= %f, %f \n",creal(t2), cimag(t2));
				printf("NewLog; t6= %f, %f \n",creal(t6), cimag(t6));
				printf("LAGUERRE LOG; z_R= %f \n",z_R);

				printf("LAGUERRE LOG; Valid To Middle \n");
				double complex integral_start = 0.0+0.0*I;
				double complex integral_end   = WaveNum+0.0*I;

				//laguerre_getParam_E_component(double complex kappa, int component, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2) 
				printf("BEFORE \n");
				printf("KAPPA= %f \n", 1.0);
				printf("RHO  = %f \n", rho);
				t6 = laguerre_getParam_E_component(1.0, 1, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2);
				printf("		BETWEEN LINES");
				t6 = laguerre_getParam_E_component(1.0, 0, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2);
				printf("WITHIN \n");
				t6 = integrate_simpsons_rule(integral_start, integral_end, 31, laguerre_getParam_E_component, 0, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2);	//E_x
				printf("BEYOND \n");
				t7 = integrate_simpsons_rule(integral_start, integral_end, 31, laguerre_getParam_E_component, 1, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2);	//E_y
				t8 = integrate_simpsons_rule(integral_start, integral_end, 31, laguerre_getParam_E_component, 2, rho, phi, z, WaveNum, l, p, z_R, w0, t1, t2);	//E_z

				printf("NewLog; t6= %f, %f \n",creal(t6), cimag(t6));
				printf("NewLog; t7= %f, %f \n",creal(t7), cimag(t7));
				printf("NewLog; t8= %f, %f \n",creal(t8), cimag(t8));

				printf("LAGUERRE LOG; Valid To Near End \n");

				j=3*i;
				cvMultScal_RVec(t6,ex,v1);
				cvMultScal_RVec(t7,ey,v2);
				cvMultScal_RVec(t8,prop,v3);
				cvAdd2Self(v1,v2,v3);
				ctemp = 1.0;
				cvMultScal_cmplx(ctemp,v1,b+j);
				printf("LAGUERRE LOG; Valid To Far End");
			}
		case B_LMINUS:
		case B_DAVIS3:
		case B_BARTON5:
