...
mode_sequence = ["View_Beam"]; % Sweep_Particle_In_Planes_Forces, Time_Simulation, Time_Simulation_Multiple
%%MODES
for mode = mode_sequence
    ...
    elseif(mode == "Time_Simulation")
        ...
        Time_Simulation(initial_position, initial_velocity, t_max, dt, radius, laguerreOrder, 1)
    ...
    elseif(mode == "Time_Simulation_Multiple")
        ...
        for dist_iter = 1:distNum
            ...
            for rad_iter = 1:radNum
                ...
                disp("Figure "+ figNum + " : radius = "+radius+ ", distance = "+ distance)
                dispStr = dispStr +"Figure "+ figNum + " : radius = "+radius+ ", distance = "+ distance + newline;
                Time_Simulation(initial_position, initial_velocity, t_max, dt, radius, laguerreOrder, figNum)
            end
        end
        disp(dispStr)
    end
...
function output = Time_Simulation(initial_position, initial_velocity, t_max, dt, radius, laguerreOrder, figNum)
    Nmax = 10;
    n_medium = 1.0;
    n_particle = 1.55;
    ...
    Plot_Force_Arrows(coords, forces, force_scaling_type, radius, figNum)
end



function output = View_Beam(view_range, Nmax, laguerreOrder_target_lower, laguerreOrder_target_upper)
    n_medium = 1.3; 
    n_particle = 1.6;
    wavelength0_base = 1064e-9;         %Vacuum wavelength
    wavelength_medium_base = wavelength0_base / n_medium;
    ...
    for laguerreOrder_iter = laguerreOrder_target_lower:laguerreOrder_target_upper
        ...
        title("Laguerre beam order "+laguerreOrder_iter+", Nmax="+Nmax);
...
% DAVID
function output = Plot_Force_Arrows(coords, forces, force_scaling_type, radius, iter)
    ...
    elseif length(coords(1,:)) == 3
        ...
        % scatter3(coords)
        zlim([padding * coords(1,3)-5e-7, padding * coords(end,3)+5e-7])
        zlabel("z /m")

    end
    
    
    dist = max(coords(:,1)) * padding;
    xlim([-dist, dist])
    ylim([-dist, dist])
    xlabel("x /m")
    ylabel("y /m")
    title("Radius = "+radius+"m")
    savefig("Figure"+iter+".fig")
    hold off;
end
...
function output = Show_Particle_Forces(laguerreOrder, distance, radius, particle_iter_max)
    ...
    Plot_Force_Arrows(coords, forces, "scale", radius, 1)
end
...
function output = Sweep_Particle_In_Planes_Forces(laguerreOrder, target_rad, variation_rad, rad_iter_max, target_dist, variation_dist, dist_iter_max, ang_iter_max, ang_max, z_min, z_max, z_iter_max)
    ...
    for rad_iter = 1:rad_iter_max
        ...
        Plot_Force_Arrows(coords, forces, "norm", radius, rad_iter) % scale, norm, xy
