...
. scale_length = FLOAT: Length that is relevent to the layout wanted (e.g. a width, radius, etc)
...
function output = FetchParticleOrigin(layout_type, scale_length, particle_total, particle_index, offset)
    if(layout_type == "line")
        ...
        output = [(particle_index-1)*spacing; 0; 0];    
    elseif(layout_type == "circle")
        %Spread evenly along the circumference of a circle in the Z=0 plane
        theta = 2*pi/particle_total;
        output = [scale_length*cos(theta*(particle_index-1 +offset));scale_length*sin(theta*(particle_index-1 +offset));0];
    else
